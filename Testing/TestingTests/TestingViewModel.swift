  ...
  override func setUp() {
	...
	func testColorIsRedWhenHexStringIsFF0000_async() {
		...
		viewModel.color.asObservable()
			.skip(1) // skip 1srt b/c Driver will replay the first element on subscription
			.subscribe(onNext: {
				result = $0
				expect.fulfill()
			})
	...
	// same test as before, but using RxBlocking
	...
	func testColorNameIsRayWenderlichGreenWhenHexStringIs006636() throws {
		...
		XCTAssertEqual(try colorNameObservable.toBlocking(timeout: 1.0).first()!, "rayWenderlichGreen")
	}
}
